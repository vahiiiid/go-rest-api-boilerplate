...
GRAB includes built-in Prometheus metrics for monitoring API performance and health. The metrics endpoint exposes real-time data about HTTP requests, latency, error rates, and resource usage.
...
| `http_request_size_bytes` | Histogram | Request payload size in bytes |
| `http_response_size_bytes` | Histogram | Response size in bytes |

**Configuration:**

Metrics collection can be enabled/disabled in `configs/config.yaml`:
...
Monitor your API with Prometheus and visualize metrics in Grafana dashboards:
...
For advanced monitoring setup and dashboard examples, see the [Monitoring Guide](https://vahiiiid.github.io/go-rest-api-docs/).
...
### Docker Container Issues on Windows

**Problem**: Container fails with `exec ./scripts/app_entrypoint.sh: no such file or directory`

**Cause**: Git on Windows may checkout shell scripts with CRLF line endings instead of LF, which Linux containers cannot execute.

**Solution**: The Dockerfile automatically converts line endings, but if you still encounter issues:
...
The project includes a `.gitattributes` file that enforces LF line endings for shell scripts to prevent this issue.
